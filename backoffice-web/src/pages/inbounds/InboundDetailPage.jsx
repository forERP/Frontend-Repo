          ...
          <div className="modal">
            ...
            >
              ...
              <div className="form-group">
                ...
                <input
                  type="text"
                  placeholder={departForm.carrierCode === DUMMY_CARRIER_CODE ? '더미 송장번호가 자동 생성됩니다.' : '송장번호를 입력해 주세요.'}
                  value={departForm.trackingNumber}
                  readOnly={departForm.carrierCode === DUMMY_CARRIER_CODE}
                  onChange={e => setDepartForm(prev => ({ ...prev, trackingNumber: e.target.value }))}
                  required
                />
                {departForm.carrierCode === DUMMY_CARRIER_CODE && (
                  <div style={{ marginTop: 8, display: 'flex', gap: 8, alignItems: 'center' }}>
                    <button
                      type="button"
                      className="btn-secondary"
                      onClick={() => setDepartForm(prev => ({ ...prev, trackingNumber: buildDummyTrackingNumber() }))}
                      disabled={submitting}
                    >
                      더미 송장 재생성
                    </button>
